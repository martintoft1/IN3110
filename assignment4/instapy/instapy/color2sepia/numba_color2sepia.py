...
class NumbaColor2Sepia(ip.color_2):
    # 4.2:
    def sepia_filter(self, image):
        """
        Method for converting a image to a sepia image

        args:
            image (integer 3D array): The image that is to be converted

        returns:
            image (integer 3D array): The sepia image
        """
        # Make the sepia image
        image = self.make_sepia_filter(image) 
    ...
    def make_sepia_filter(image):
        """
        The actual method that utilizes numba to create a sepia image from an image
    ...
    def report_sepia_filter(self, input_filename, output_directory,  *report_files):
        """
        Method for automatically writing and saving a report of the sepia_filter-function on a given image with the numba-implementation
        ...
        self.save_report(report, "numba_report_color2sepia.txt", output_directory)